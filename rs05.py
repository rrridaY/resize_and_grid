...
def process(
    ...
    for i, page in enumerate(src_doc):
        ...
        # クリップ指定がある場合
        if option_obj.get(i/2+1) is not None:
            clip_height = option_obj.get(i/2+1)
        ...
        GRID_OFFSET = 10
        # 左側の余白に罫線を描画
        for y in range(clip_height, int(orig_rect.height), grid_width):
    ...
    print(f"元のページ数: {len(src_doc)},変換後のページ数: {len(new_doc)}")
